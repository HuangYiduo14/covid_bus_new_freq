...
def genearte_freq_graph(line_dict, station_loc, distance_dict, line_freq):
    ...
    for station, st_loc in station_loc.items():
        ...
        for line_name in station_dict[station]:
            G.add_edge(str(station)+'_enter', str(station)+'_'+line_name, line_name=line_name, length=0, frequency = BigFreq, capacity=LineCap,type='boarding')
            G.add_edge(str(station)+'_'+line_name, str(station)+'_exit', line_name=line_name, length =0, frequency = BigFreq, capacity=LineCap,type='alighting')
